...
export const addVideo = async (req, resp, next) => {
  try {
    ...
    const { title, desc, views, tags, likes, dislikes } = req.body;
    const files = req.files;
    const fileUri1 = getDataUri(files[0]);
    const fileUri2 = getDataUri(files[1]);
    const myCloud1 = cloudinary.uploader.upload(fileUri1.content);
    const myCloud2 = cloudinary.uploader.upload(
      fileUri2.content,
      { resource_type: "video" },
      (error, result) => {
    ...
    const newVideo = new Video({
      ...
      userImgUrl: userDetails.imgUrl,
      imgUrl: (await myCloud1).url,
      videoUrl: (await myCloud2).url,
    });
...
export const addView = async (req, resp, next) => {
  try {
    ...
    const result1 = await Video.findByIdAndUpdate(videoId, {
      $inc: { views: 1 },
    });
